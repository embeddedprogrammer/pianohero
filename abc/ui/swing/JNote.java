...
public class JNote extends JNoteElementAbstract {
	...
	protected void onBaseChanged() {
		...
		Dimension2D glyphDimension = metrics.getGlyphDimension(getNotationContext());
		//note glyph is used for vertical position of graces notes
		Dimension2D noteGlyphDimension = metrics.getGlyphDimension(ScoreMetrics.NOTATION_CONTEXT_NOTE);

		Point2D base = getBase();
		//System.out.println("gd:" + noteGlyphDimension.getHeight());
		int noteY = 0;
		if (note.isRest())
...
//			double noteY2 = (double)(base.getY()-getOffset(note)*((double)noteGlyphDimension.getHeight())
//					- ((double)noteGlyphDimension.getHeight())/2 + ((double)glyphDimension.getHeight())/2);
//			System.out.println("" + noteY + " vs. " + noteY2);
		}
...
//		System.out.println("JNote width: " + graceNotesWidth + " + " + accidentalsWidth + " + " + glyphDimension.getWidth() + " + " + extraWidth + " = " + m_width);
	...
	protected void calcDotsPosition() {
		if ((note.countDots() != 0)
			...
			Dimension2D glyphDimension = getMetrics().getGlyphDimension(getNotationContext());
