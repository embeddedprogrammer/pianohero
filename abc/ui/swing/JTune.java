...
public class JTune extends JScoreElementAbstract {
	...
	public Hashtable getScoreElements()
	{
...
//	}
	...
	public double getStaffWidth()
	{
		if(currentStaffLine == null)
			return 0;
		return currentStaffLine.getWidth();
	}
	
	public void setMinStaffWidth(double min)
	{
		if(currentStaffLine != null)
			currentStaffLine.setMinWidth(min);
	}
...
//		System.out.println("element: " + element + ", width: " + width);
...
//				System.out.println("metrics: " + getMetrics().getNotesSpacing());
//				System.out.println("engraver: " + getEngraver().getNoteSpacing(element));
			}
		}
...
//					additionnalCurves.add(new Point2D[] {p2d[START],newPtControl,p2d[END]});
	...
	private JStaffLine initNewStaffLine() {
		...
		if ((currentKey!=null) && !hasKeyChange) {
			...
			cursor.setLocation(cursorNewLocationX, cursor.getY());
			m_scoreElements.put(currentKey.getReference(), sk);
		}
		if ((currentTime!=null) && (m_staffLines.size()==0) && !hasTimeChange) {
			try {
				...
				int cursorNewLocationX = (int)(cursor.getX() + width + getMetrics().getNotesSpacing());
				cursor.setLocation(cursorNewLocationX, cursor.getY());
				m_scoreElements.put(currentTime.getReference(), sig);
			}
			catch (Exception e) {
